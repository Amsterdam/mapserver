        ...
        if layer["group"] == 'pand_loopafstand':
            ...
            for range in loopafstanden_ranges_selected:
                with block("LAYER"):
                    ...
                    p("INCLUDE", "connection/dataservices.inc")
                    p("DATA", "geometrie FROM (SELECT bol.id, bol.geometrie, bol.fractie_omschrijving, lac.loopafstand_categorie_omschrijving FROM public.huishoudelijkafval_bag_object_loopafstand_v2_v2 bol INNER JOIN public.huishoudelijkafval_loopafstand_categorie_v2_v2 lac ON bol.loopafstand_categorie_id = lac.id WHERE 1=1) rest USING srid=28992 USING UNIQUE id")
