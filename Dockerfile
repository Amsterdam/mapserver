...
# Setup build env
RUN mkdir /build
RUN apt-key adv --recv-keys --keyserver-options http-proxy=$http_proxy --keyserver hkp://keyserver.ubuntu.com:80 16126D3A3E5C1192    \
...
RUN apt-key adv --recv-keys --keyserver-options http-proxy=$http_proxy --keyserver hkp://keyserver.ubuntu.com:80 314DF160    \
  && apt-get update
