            ...
            if "url" in item:
                ...
                if m:
                    ...
                    if "layers" in item:
                        layers = item["layers"]
                        for layer in layers:
                            if layer not in map_value and not (
                                GROUPS_NAME in map_value
                                and layer in map_value[GROUPS_NAME]
                            ):
                                warn(
                                    f"Missing layer {layer} for {mapfile} in map-panel-layers.js"
                                )
                                stop_item = True
                    ...
                    if "legendItems" in item:
                        ...
                        for legend_item in legend_items:
                            ...
                            selectable = (
                                item["selectable"] if "selectable" in item else False
                            )
                            if "layer" in legend_item:
                                ...
                                if isinstance(layer_li, list):
                                    layer_li = layer_li[0]
                                    warn(
                                        f"legendItem {li_title} contains list of layer {layer_li} in map-panel-layers.js"
                                    )
                                if layer_li not in map_value:
                                    warn(
                                        f"Missing layer {layer_li} for legendItem {li_title} in map-panel-layers.js"
                                    )
                            elif selected_layer is not None:
                                layer_li = selected_layer
                            else:
                                warn(
                                    "Missing or unknown layer for legendItem {li_title}"
                                )
                            if selectable:
                                pass
                            else:
                                ...
                                if layer_li not in map_value:
                                    if (
                                        GROUPS_NAME in map_value
                                        and layer_li in map_value[GROUPS_NAME]
                                    ):
                                        layer_li = map_value[GROUPS_NAME][layer_li]
                                    else:
                                        warn(
                                            f"Missing layer {layer_li} in map {mapfile} in map-panel-layers.js"
                                        )
                                        continue
                                layers_li = (
                                    layer_li
                                ...
                                found_in_layer = list(
                                    filter(
                                        lambda x: li_title in map_value[x], layers_li
                                    )
                                )
                                if len(found_in_layer) == 0:
                                    warn(
                                        f"Missing class name {li_title} for layer {layer_li}, map {mapfile} in map-panel-layers.js"
                                    )
