...
with block("MAP"):
    ...
    with block("WEB"):
        with block("METADATA"):
            ...
            q(
                "ows_abstract",
                "Vestigingen in de gemeente Amsterdam uit het Handelsregister",
            )
    ...
    for code, descr in sbi:
        ...
        with block("LAYER"):
            ...
            with block("METADATA"):
                q("ows_title", slug)
                q("ows_group_title", "vestigingen_cbs")
            ...
            with block("CLASS"):
                with block("STYLE"):
                    p("SYMBOL", "cirkel_oranje")
                    p("SIZE 16")

        with block("LAYER"):
            p("NAME", slug + "_label")
            p("GROUP", "vestigingen_cbs_label")
            p("INCLUDE", "connection/dataservices.inc")
            p("DATA", sql)
            p("TYPE POINT")
            # p("MINSCALEDENOM 100")
